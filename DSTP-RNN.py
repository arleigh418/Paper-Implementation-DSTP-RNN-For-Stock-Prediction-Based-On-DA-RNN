...
class Encoder(nn.Module):
    ...
    def forward(self, X ,y_prev):
        ...
        for t in range(self.T - 1):
            ...
            #Phase two attention for DSTP-RNN Paper
...
class DSTP_rnn(nn.Module):
    ...
    def __init__(self, X, y, T,
        ...
        # Training set
        self.train_timesteps = int(self.X.shape[0] * 0.8) #改
        self.input_size = self.X.shape[1]
        self.dev_data = int(self.X.shape[0] * 0.1)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            while (idx < self.train_timesteps):
                ...
                if record_loss == 0:
                    record_loss = loss
                elif record_loss != 0 and loss < record_loss:
                    record_loss = loss
                    loss_no_update = 0
                else:
                    loss_no_update+=1

                if loss_no_update == 10 and n_iter != 0: 
            ...
            if epoch % 1 == 0:
                print ("Epochs: ", epoch, " Iterations: ", n_iter, " Loss: ", loss)
          
            val_result = model.val()
     
            if val_record ==0:
                val_record = val_result
            elif val_record !=0 and val_record > val_result:
                val_record = val_result
                val_no_update = 0
                print('New Val Record')
            elif val_record !=0 and val_record < val_result and val_no_update<10:
                val_no_update += 1
                print('No update')
    ...
    def val(self):
        ...
        y_val_price = np.zeros(self.dev_data)

        i = 0
        while i < len(y_val_price):
            batch_idx = np.array(range(len(y_val_price)))[i : (i + self.batch_size)]
            # print(batch_idx)
            ...
            y_val_price = self.Decoder(input_encoded, y_history) #cuda
            
            y_val_price = y_val_price[i:(i + self.batch_size)]
            y_val_price = y_val_price.cpu().detach().numpy()[:, 0]
            i += self.batch_size
        y_val_real = list(self.y[model.train_timesteps:model.train_timesteps+model.dev_data])
       
        val_result = count_values(y_val_real,y_val_price)
        return val_result
    ...
    def test(self, on_train=False):
        ...
        if on_train:
            y_pred_price = np.zeros(self.train_timesteps -self.dev_data- self.T + 1)
            
        else:
            y_pred_price = np.zeros(self.X.shape[0] -self.dev_data- self.train_timesteps)
        ...
        while i < len(y_pred_price):
            ...
            for j in range(len(batch_idx)):
                if on_train:
                    ...
                    X[j, :, :] = self.X[range(batch_idx[j] +self.train_timesteps+ self.dev_data - self.T, batch_idx[j] +self.train_timesteps+ self.dev_data - 1), :]
                    y_history[j, :] = self.y[range(batch_idx[j] +self.train_timesteps+ self.dev_data - self.T,  batch_idx[j]+ self.train_timesteps+self.dev_data - 1)]
            ...
            y_pred_price = self.Decoder(input_encoded, y_history) #cuda
...
model = DSTP_rnn(X, y, 10 , 128, 128, 128, 0.01, 8000)
model.train()
